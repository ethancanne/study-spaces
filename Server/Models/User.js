...
* @date   11/15/2021
*/
UserSchema.statics.createUser = async(unverifiedUser) =>{
...
UserSchema.statics.getById = async (userId) =>{
  ...
  try {
    userRecord = await User.findOne({ _id: convertedUserId }).exec();
  } catch(error) {
...
* @date   10/22/2021
*/
  UserSchema.statics.getByEmail = async (userEmail) =>{
  // GET THE USER BASED ON THE GIVEN EMAIL.
  let userRecord = false;
  try {
    userRecord = await User.findOne({ email: userEmail }).exec();
  } catch(error) {
...
//     this.object = userSchema.toObject();
//   }
