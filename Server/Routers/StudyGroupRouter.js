...
class StudyGroupRouter {
    ...
    static serveRoutes(server, authenticator) {
        ...
        server.post(
            ...
            Validator.validateOneTimeMeeting,
        ...
        server.post(
            Routes.StudyGroup.EditMeeting,
            authenticator.protectRoute(),
            Validator.validateEditMeeting,
            StudyGroupRouter.editOneTimeMeeting
        );
        ...
        server.post(
            ...
            authenticator.protectRoute(),
            Validator.validateEditStudyGroup,
            StudyGroupRouter.editStudyGroup
        );
        ...
        server.post(
            ...
            authenticator.protectRoute(),
            Validator.validateSetRecurringMeeting,
    ...
    static async editOneTimeMeeting(request, response) {
        ...
        if (request.body.date != meeting.getDate()) {
            if (request.body.date == null || request.body.date == "" || typeof date !== "object") {
                return response.json({ message: "Date is a required attribute." });
            } else {
                await meeting.setDate(request.body.date);
            }
        }

        if (request.body.time != meeting.getTime()) {
            if (request.body.time == null || request.body.time == "") {
                return response.json({ message: "Time is a required attribute." });
            } else {
                await meeting.setTime(request.body.time);
            }
        }

        if (request.body.day != meeting.getDay()) {
            if (request.body.day == null || request.body.day == "") {
                return response.json({ message: "Day is a required attribute." });
            } else {
                await meeting.setDay(request.body.day);
            }
        }

        if (request.body.details != meeting.getDetails()) {
            await meeting.setDetails(request.body.details);
        }

        if (request.body.location != meeting.getLocation()) {
            await meeting.setLocation(request.body.location);
        }

        if (request.body.roomNumber != meeting.getRoomNumber()) {
            await meeting.setRoomNumber(request.body.roomNumber);
        }
    ...
    static async editStudyGroup(request, response) {
        ...
        if (request.body.isTutorGroup != studyGroup.getIsTutorGroup()) {
            if (studyGroup.getIsTutorGroup()) {
                await studyGroup.makeStudyGroup();
            } else {
                await studyGroup.makeTutorGroup();
            }
        }
        ...
        if (request.body.name != studyGroup.getName()) {
            ...
            await studyGroup.setName(request.body.name);
        }

        //Check for change in study group subject
        if (request.body.subject != studyGroup.getSubject()) {
            if (request.body.subject == null || request.body.subject == "") {
                return response.json({ message: ResponseMessages.StudyGroup.ErrorNullStudyGroupInput });
            }
            await studyGroup.setSubject(request.body.subject);
        }
    ...
    static async setRecurringMeeting(request, response) {
        ...
        if (recurringMeetingExists) {
            ...
            if (meetingUpdated) {
                ...
                let postWasCreated = undefined;
                try {
                    postWasCreated = await feed.addPost(
